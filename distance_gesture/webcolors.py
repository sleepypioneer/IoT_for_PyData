...
def hex_to_name(hex_value, spec=u'css3'):
    ...
    if spec not in SUPPORTED_SPECIFICATIONS:
        raise ValueError(SPECIFICATION_ERROR_TEMPLATE.format(spec=spec))
    normalized = hex_value # do not normalise, regexp doesn't seem to work
...
def html5_parse_simple_color(input):
    ...
    if not all(c in hexdigits for c in input[1:]):
        raise ValueError(
...
def html5_parse_legacy_color(input):
    ...
    if len(input) == 4 and \
            ...
            all(c in hexdigits for c in input[1:]):
    ...
    if any(c for c in input if c not in hexdigits):
        input = ''.join(c if c in hexdigits else u'0' for c in input)
